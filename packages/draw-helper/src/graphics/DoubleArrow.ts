...
class DoubleArrow extends Graphics {
  type = 'doubleArrow'
  ...
  start(options: GraphicsEntityOptions = {}) {
    ...
    handler.setInputAction((movement: any) => {
      ...
      if (!cartesian) return
      // 如果已经创建实体，并且坐标位置数量大于等于4，结束绘制
      if (this.entity && this.positions.length >= 4) {
        this.complete()
      } else {
        // 第一次点击添加坐标位置
    ...
    handler.setInputAction((movement: any) => {
      ...
      if (this.positions.length === 0) {
        ...
        if (this.positions.length >= 4) {
          ...
          this._tooltip.showAt(position, '单击左键绘制下一个点')
        }
        // 添加图形
        if (!this.entity) {
  ...
  computeArrowPositions(pos: Cesium.Cartesian3[]) {
    ...
    const points = this.convertCartesianToDegreesArray2(pos)
    const positions = algorithm.doubleArrow(points)
  ...
  getLerpArrow(
    ...
    const center = bones[0]
    const pos3 = bones[1]
    const pos4 = bones[2]
    // 坐标3
    const p3 = new Cesium.Cartesian3()
    // 坐标4
    const p4 = new Cesium.Cartesian3()
    ...
    if (isLessThanStart) {
      ...
      if (t <= 0) t = 0
      if (t >= 1) t = 1
      Cesium.Cartesian3.lerp(center, pos3, t, p3)
      Cesium.Cartesian3.lerp(center, pos4, t, p4)
      positions = this.computeArrowPositions([...bottomPoint, p3, p4])
    }
    return positions
  }
