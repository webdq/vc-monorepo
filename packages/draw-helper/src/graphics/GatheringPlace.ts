...
class GatheringPlace extends Graphics {
  type = 'gatheringPlace'
  fittingCount = 100
  t = 0.4
  constructor(
  ...
  start(options: GraphicsEntityOptions = {}) {
    ...
    handler.setInputAction((movement: any) => {
      ...
      if (!cartesian) return
      // 如果已经创建实体，并且坐标位置数量大于等于3，结束绘制
      if (this.entity && this.positions.length >= 3) {
        this.complete()
      } else {
        // 第一次点击添加坐标位置
  ...
  computeArrowPositions(pos: Cesium.Cartesian3[]) {
    ...
    const points = pos.map((item) => {
      const { longitude, latitude } = this.convertCartesianToDegrees(item)
      return [longitude, latitude]
    })
    const positions = algorithm.gatheringPlace(points)
