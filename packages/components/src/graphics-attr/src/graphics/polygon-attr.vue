<template>
  <div class="graphics-form">
    ...
    >
      ...
      <template v-if="formData.extrudedHeight">
        <a-form-item label="是否顶部封口">
          <a-switch
            v-model:checked="formData.closeTop"
            class="form-switch"
            @change="attrChange('closeTop')"
          />
        </a-form-item>
        <a-form-item label="是否底部封口">
          <a-switch
            v-model:checked="formData.closeBottom"
            class="form-switch"
            @change="attrChange('closeBottom')"
          />
        </a-form-item>
      </template>
...
} = useAttrsChange(formData, 'polygon', emit)
</script>
